test_that("parse_consultar_procedimento_response parseia a fixture real", {
  ...
  expect_equal(res$ProcedimentoFormatado, "0011108545.000056/2022-49")
...
test_that("andamentos são ancorados na subárvore correta (bug Sigla/Descricao)", {
  ...
  expect_equal(res$AndamentoGeracao_SiglaUsuario, "auristela.souza")
  expect_equal(res$AndamentoGeracao_NomeUsuario, "Auristela Ferreira De Souza")
  ...
  expect_equal(res$UltimoAndamento_SiglaUsuario, "hayanne.carneiro")
})
...
test_that("arrays viram colunas-lista de tibbles", {
  ...
  expect_equal(res$Interessados[[1]]$Sigla, "romero.amorim")
})
