test_that("consultar_procedimentos empilha resultados num tibble", {
  ...
  res <- consultar_procedimentos(c("0011108545.000056/2022-49", "0030600043.002462/2024-05"),
                                 config = sei_config())
  expect_s3_class(res, "tbl_df")
  expect_equal(nrow(res), 2L)
  expect_equal(res$protocolo,
               c("0011108545.000056/2022-49", "0030600043.002462/2024-05"))
...
test_that("erro em um protocolo não derruba o lote (registra em 'erro')", {
  ...
  res <- consultar_procedimentos(c("0011108545.000056/2022-49", "BAD"),
                                 config = sei_config())
  expect_equal(nrow(res), 2L)
